 ...
 * @return {Array}
 */
var defaultTokenizer = function (text,toLearn) {
  ...
  sanitized.forEach(token => {
    ...
    if(!toLearn){
      var matches = stringSimilarity.findBestMatch(token,Object.keys(this.vocabulary));
    if(matches.bestMatch.rating>=0.5){
      stemmed=matches.bestMatch.target
    }
    }
    stemsRemoved.push(stemmed)
  });
  return stemsRemoved;
}
...
Naivebayes.prototype.learn = function (text, category) {
  ...
  var tokens = self.tokenizer(text,true)
...
Naivebayes.prototype.categorize = function (text) {
  ...
  .forEach(function (category) {
    ...
    .forEach(function (token) {
      ...
      logProbability += frequencyInText * tokenProbability
    });
    logProbability=Math.abs(logProbability);
    if (logProbability > maxProbability) {
      maxProbability = logProbability
      chosenCategory = category
    }
    else if(logProbability === maxProbability){
      count++;
      console.log(count);
      
    }
  })  
  if(count+1 === Object.keys(self.categories).length)
    chosenCategory='unknown'
...
Naivebayes.prototype.probabilities = function(text) {
  ...
  .map(function (category) {
    ...
    //    mapped to this category
    var categoryProbability = self.docCount[category] / self.totalDocuments
    console.log(category,categoryProbability);
